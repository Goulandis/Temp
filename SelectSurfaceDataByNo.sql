delimiter //
-- 根据断面编号搜索水纹数据
create procedure SelectSurfaceDataByNo(
...
begin
	if in_surface_no is not null then
    begin
        ...
        begin
			...
			if in_time_point_start is null and in_time_point_end is null then
				select * from surface_data where surface_no = in_surface_no;
			-- 场景2-输入断面名字和某时见戳查询指定断面指定时间的水纹数据
			elseif in_time_point_start is not null and in_time_point_end is null then
				select * from surface_data where surface_no = in_surface_no and time_point = in_time_point_start;
			-- 场景3-输入断面名字和一个时间段查询这个时间段内这个断面的所有的水文数据
			elseif in_time_point_start is not null and in_time_point_end is not null then
				select * from surface_data where surface_no = in_surface_no and ascii(time_point) >= ascii(in_time_point_start)
